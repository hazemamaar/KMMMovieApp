...
fun MovieApp() {
    ...
    Scaffold(scaffoldState = scaffoldState,
        ...
        NavHost(navController = navController, modifier = Modifier.padding(innerPaddings), startDestination = Home.routeWithArgs ){
            composable(Home.routeWithArgs){
                val homeViewModel:HomeViewModel= koinViewModel()

                HomeScreen(uiState = homeViewModel.uiState, loadNextMovies = {
                    homeViewModel.loadMovies(forceReload = it)
                    Log.e("loading", "${homeViewModel.loadMovies(forceReload = it)}" )
                }, navigateToDetail ={
